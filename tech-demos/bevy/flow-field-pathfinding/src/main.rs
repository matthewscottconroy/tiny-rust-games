...
//!
//! Instead of running A* per agent, this demo pre-computes a "flow field" — a
//! direction vector at every grid cell pointing toward a goal — so thousands of
//! agents can look up their cell's direction in O(1) per frame.  The field
//! itself is recomputed with a single BFS from the goal outward, costing
//! O(grid size) once per goal/wall change.
//!
//! ## Controls
