//! Mouse input demo — a reusable Bevy plugin.
//!
//! This crate is a *building block*: drop [`MouseInputPlugin`] into any Bevy app
//! with `app.add_plugins(MouseInputPlugin)` and it spawns draggable shapes that
//! respond to hover and left-drag.
//!
//! Key ideas:
...
//! - Drag state is tracked in a [`DragState`] resource; on left-press the closest
//!   entity within its radius is picked, on left-release it is dropped.
...
/// Bundles every system and resource for the mouse-input feature.
///
/// Add it with `app.add_plugins(MouseInputPlugin)`. It does **not** add
...
pub struct Draggable {
    /// The sprite's resting color, restored when not hovered.
...
pub struct DragState {
    ...
    /// Offset between the pick point and the entity centre.
    pub offset: Vec2,
}
...
fn update_hover(
    ...
    for mut text in &mut label_query {
        *text = Text::new(hovered_name.clone().unwrap_or_default());
    }
}
