//! Procedural dungeon — a reusable Bevy plugin.
//!
//! This crate is a *building block*: drop [`ProceduralDungeonPlugin`] into any
//! Bevy app with `app.add_plugins(ProceduralDungeonPlugin)` and it generates and
//! renders a BSP dungeon that regenerates on SPACE. Tune it through the
//! [`DungeonConfig`] resource without editing the plugin.
//!
//! Key ideas:
//! - A [`Rect`] represents a region of the grid; [`split_room`] bisects it along
//!   a chosen axis, returning two child rects only when both children meet the
...
//!   L-shaped corridor. It is a pure function, unit-testable without a World.
...
/// Bundles every system and resource for the dungeon feature.
///
/// Add it with `app.add_plugins(ProceduralDungeonPlugin)`. It does **not** add
/// `DefaultPlugins`, so the host app stays in control of window and rendering
/// setup.
pub struct ProceduralDungeonPlugin;

impl Plugin for ProceduralDungeonPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<DungeonConfig>()
            .init_resource::<DungeonSeed>()
...
/// Tunable parameters for the dungeon generator and renderer. Override before
/// adding the plugin, e.g.
/// `app.insert_resource(DungeonConfig { cols: 64, ..default() })`.
...
pub struct DungeonConfig {
    ...
    pub rows: usize,
    /// Rendered size of one tile in pixels.
    pub tile_px: f32,
    /// Minimum room dimension enforced during BSP splitting.
    pub min_room: usize,
}
...
/// Builds a `rows × cols` dungeon grid from `seed` via BSP splitting, carving
...
/// The current dungeon seed. Advancing it (via [`lcg_next`]) regenerates.
#[derive(Resource)]
...
/// Spawns the tile sprites for `grid` and their `TileSprite` markers.
fn render_grid(commands: &mut Commands, grid: &[Vec<Tile>], config: &DungeonConfig) {
    let tile_px = config.tile_px;
    let offset_x = -(config.cols as f32 * tile_px) / 2.0 + tile_px / 2.0;
    let offset_y = (config.rows as f32 * tile_px) / 2.0 - tile_px / 2.0;
...
/// Builds and renders the dungeon grid, plus the seed label.
fn spawn_dungeon(mut commands: Commands, seed: Res<DungeonSeed>, config: Res<DungeonConfig>) {
    let grid = build_dungeon(seed.0, config.cols, config.rows, config.min_room);
    render_grid(&mut commands, &grid, &config);
...
mod tests {
    ...
    #[test]
    fn build_dungeon_has_expected_dimensions() {
        let grid = build_dungeon(7, 48, 28, 5);
        assert_eq!(grid.len(), 28);
        assert_eq!(grid[0].len(), 48);
    }

    #[test]
    fn config_default_values_are_sane() {
        let c = DungeonConfig::default();
        assert!(c.cols > 0 && c.rows > 0);
        assert!(c.tile_px > 0.0);
        assert!(c.min_room > 0);
    }
}
