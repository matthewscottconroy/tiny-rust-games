//! Simulated 2D lighting — a reusable Bevy plugin.
//!
//! This crate is a *building block*: drop [`Lighting2dPlugin`] into any Bevy app
//! with `app.add_plugins(Lighting2dPlugin)` and it renders a tile grid lit by
//! distance-based light intensity. Tune the layout through the
//! [`Lighting2dConfig`] resource without editing the plugin's internals.
//!
//! Key ideas:
...
/// Bundles every system and resource for the 2D lighting feature.
///
/// Add it with `app.add_plugins(Lighting2dPlugin)`. It does **not** add
/// `DefaultPlugins` (nor a `ClearColor`), so the host app stays in control of
...
/// Tunable layout parameters. Override before adding the plugin, e.g.
/// `app.insert_resource(Lighting2dConfig { player_speed: 200.0, ..default() })`.
#[derive(Resource, Clone, Copy, Debug)]
pub struct Lighting2dConfig {
    ...
    pub tile_px: f32,
    /// Player light movement speed in units per second.
...
mod tests {
    ...
    fn config_default_matches_constants() {
        let c = Lighting2dConfig::default();
        assert_eq!(c.cols, COLS);
        assert_eq!(c.rows, ROWS);
        assert_eq!(c.tile_px, TILE_PX);
        assert_eq!(c.player_speed, PLAYER_SPEED);
    }
}
