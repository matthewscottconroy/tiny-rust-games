//! Line-of-sight — a reusable Bevy plugin.
//!
//! This crate is a *building block*: drop [`LineOfSightPlugin`] into any Bevy
//! app with `app.add_plugins(LineOfSightPlugin)` and it renders a walkable grid
//! with Bresenham ray-cast visibility from a movable player.
//!
//! Key ideas:
...
//!
//! Tune grid dimensions, tile size, and sight radius through the
//! [`LineOfSightConfig`] resource without editing the plugin's internals.
//!
...
/// Bundles every system and resource for the line-of-sight feature.
///
/// Add it with `app.add_plugins(LineOfSightPlugin)`. It does **not** add
...
impl Plugin for LineOfSightPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<LineOfSightConfig>()
...
/// `app.insert_resource(LineOfSightConfig { sight_radius: 12, ..default() })`.
...
/// The walkable grid (`true` = floor, `false` = wall).
#[derive(Resource)]
pub struct WalkGrid(pub Vec<Vec<bool>>);

/// The player's current grid cell.
#[derive(Resource)]
...
/// Marks the player sprite entity.
#[derive(Component)]
pub struct PlayerMarker;

// --- Setup ---

fn cell_to_world(cell: IVec2, config: &LineOfSightConfig) -> Vec3 {
...
fn setup(mut commands: Commands, mut revealed: ResMut<Revealed>, config: Res<LineOfSightConfig>) {
    ...
    for (r, row) in grid.iter().enumerate() {
        for (c, _) in row.iter().enumerate() {
            ...
            commands.spawn((
                ...
                Transform::from_translation(cell_to_world(cell, &config)),
                TileCell(cell),
            ));
        }
    }
    ...
    commands.spawn((
        Sprite {
            ...
            custom_size: Some(Vec2::splat(config.tile_px * 0.6)),
            ..default()
        },
        Transform::from_translation(cell_to_world(start, &config).with_z(1.0)),
        PlayerMarker,
    ));
...
fn update_visibility(
    ...
    let rows = grid.0.len() as i32;
    let cols = grid.0[0].len() as i32;
    let radius = config.sight_radius;
...
fn sync_player(
    ...
    if let Ok(mut t) = q.single_mut() {
        t.translation = cell_to_world(player.0, &config).with_z(1.0);
    }
}
...
mod tests {
    ...
    fn config_default_matches_documented_values() {
        let c = LineOfSightConfig::default();
        assert_eq!(c.cols, 30);
        assert_eq!(c.rows, 20);
        assert_eq!(c.tile_px, 26.0);
        assert_eq!(c.sight_radius, 8);
    }
}
