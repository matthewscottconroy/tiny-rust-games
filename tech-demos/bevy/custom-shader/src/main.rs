//! # Custom Shader Demo
//!
//! Demonstrates how to extend Bevy's rendering pipeline with a custom `Material2d`
//! backed by a hand-written WGSL fragment shader. The shader produces an animated
//! ripple pattern driven by a `time` uniform that is updated each frame.
//!
//! Key concepts:
...
#[derive(Resource)]
struct RippleHandle(Handle<RippleMaterial>);
...
fn main() {
    App::new()
        ...
        .add_plugins(Material2dPlugin::<RippleMaterial>::default())
