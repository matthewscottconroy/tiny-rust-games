//! Runnable entry point for the parallax-background demo.
//!
//! All the reusable logic lives in the crate library ([`parallax_background`])
//! as [`ParallaxBackgroundPlugin`]. This binary is just the thin harness that
//! boots the engine and adds the plugin — copy `lib.rs` into your own project
//! (or depend on this crate) and add the one plugin to reuse the feature.

use bevy::prelude::*;
use parallax_background::ParallaxBackgroundPlugin;

fn main() {
    App::new()
        .add_plugins(DefaultPlugins)
        .add_plugins(ParallaxBackgroundPlugin)
        .run();
}
