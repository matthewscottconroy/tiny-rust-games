//! Circle-buttons GUI demo — coloured circles that act as clickable buttons.
//!
//! This demo shows how to build a self-contained GUI component as a Bevy plugin:
//!
//! - [`CircleGuiConfig`] is a [`Resource`] inserted by the caller before the app
//!   starts; it declares how many circles to draw, their radius, spacing, and
//!   individual colours.
//! - [`CircleGuiPlugin`] reads the config, spawns the meshes, and handles all
//!   mouse interaction internally.
//! - Results flow out of the plugin through two parallel channels so callers can
//!   choose whichever suits them:
//!   - **Push**: [`CircleClicked`] events fired on every click.
...
fn main() {
    App::new()
        ...
        // External consumer: reads CircleClicked events from outside the plugin.
