//! Loot table demo — weighted random item drops without the `rand` crate.
//!
//! Key ideas:
//! - A `LootTable` is a slice of `(Item, weight)` pairs.
    ...
    App::new()
        .add_plugins(DefaultPlugins.set(WindowPlugin {
            primary_window: Some(Window {
                ...
                resolution: (720u32, 460u32).into(),
                ..default()
            }),
            ..default()
        }))
        .insert_resource(RngState(0xDEAD_BEEF_u64))
...
fn click_enemy(
    ...
    for (entity, tf, enemy) in &enemy_query {
        ...
        if (cursor - pos).abs().max_element() < enemy.half_size {
            ...
            if let Some(&item) = roll_loot(TABLE, rng.0 as u32) {
...
/// Floats drop labels upward and removes them after 1.4 seconds.
fn tick_drops(
    time: Res<Time>,
    mut commands: Commands,
