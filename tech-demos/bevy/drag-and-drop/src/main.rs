//! # Drag-and-Drop Demo
//!
//! Demonstrates cursor-to-world coordinate conversion and drag mechanics in Bevy.
//!
//! Eight colored squares are scattered around the screen. Left-clicking the
//! square nearest to (and under) the cursor picks it up, raising its Z-order
//! to bring it visually in front of all others. Holding the mouse button and
//! moving drags the square while preserving the original grab offset so the
//! square doesn't jump to the cursor. Releasing the mouse drops the square and
//! snaps its position to a 20-unit grid.
//!
//! Key techniques:
//! - `Camera::viewport_to_world_2d` for screen-to-world conversion
//! - `DragState` resource to track the active entity and grab offset
//! - Z manipulation to bring the active item to the front
...
fn end_drag(
    ...
    if let Some(entity) = drag_state.dragging.take() {
        if let Ok(mut transform) = transforms.get_mut(entity) {
            let snapped = snap_to_grid(transform.translation.truncate(), 20.0);
