//! Health and damage — a reusable Bevy plugin.
//!
//! This crate is a *building block*: drop [`HealthAndDamagePlugin`] into any
//! Bevy app with `app.add_plugins(HealthAndDamagePlugin)` and it manages an
//! enemy with a health bar, damage input, death, and respawn. Tune it through
//! the [`HealthConfig`] resource without editing the plugin's internals.
//!
//! Key ideas:
...
/// Bundles every system, message, and resource for the health/damage feature.
///
/// Add it with `app.add_plugins(HealthAndDamagePlugin)`. It does **not** add
...
/// `app.insert_resource(HealthConfig { max_hp: 250.0, ..default() })`.
#[derive(Resource, Clone, Copy, Debug)]
pub struct HealthConfig {
    /// Starting (and maximum) hit points of a freshly spawned enemy.
    pub max_hp: f32,
    /// Hit points removed per SPACE press.
    pub damage_per_hit: f32,
    /// Delay before a dead enemy respawns.
...
pub struct DamageMessage {
    /// The entity that should take damage.
    pub target: Entity,
    /// How many hit points to remove.
    pub amount: f32,
}
...
fn setup(mut commands: Commands, config: Res<HealthConfig>) {
    commands.spawn(Camera2d);
    spawn_enemy(&mut commands, &config);
    spawn_hud(&mut commands, &config);
}

/// Spawns the enemy sprite and its health-bar UI.
fn spawn_enemy(commands: &mut Commands, config: &HealthConfig) {
    commands
        .spawn((
            Node {
...
mod tests {
    ...
    fn config_default_matches_documented_values() {
        ...
        assert_eq!(c.damage_per_hit, 10.0);
    }
