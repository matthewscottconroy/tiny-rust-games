//! AABB collision detection — a reusable Bevy plugin.
//!
//! This crate is a *building block*: add [`CollisionDetectionPlugin`] to any
//! Bevy app with `app.add_plugins(CollisionDetectionPlugin)` and it spawns a set
//! of boxes that bounce around an arena, resolving overlaps each frame. Tune the
//! arena and collision tint through the [`CollisionDetectionConfig`] resource.
//!
//! Key ideas:
...
/// Bundles every system and resource for the collision-detection feature.
///
/// Add it with `app.add_plugins(CollisionDetectionPlugin)`. It does **not** add
...
impl Plugin for CollisionDetectionPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<CollisionDetectionConfig>()
            .add_systems(Startup, setup)
            .add_systems(
                Update,
                (move_boxes, detect_and_resolve_collisions, bounce_walls),
            );
    }
}
...
/// `app.insert_resource(CollisionDetectionConfig { arena_half: Vec2::new(600.0, 400.0), ..default() })`.
#[derive(Resource, Clone, Copy, Debug)]
pub struct CollisionDetectionConfig {
    /// Half-extents of the arena; boxes bounce at `±arena_half`.
    pub arena_half: Vec2,
    /// Color a box is tinted while colliding.
...
impl Default for CollisionDetectionConfig {
    fn default() -> Self {
        Self {
            arena_half: Vec2::new(480.0, 270.0),
            collision_color: Color::srgb(1.0, 0.5, 0.1),
        }
    }
}
...
pub struct Box {
    ...
    pub size: Vec2,
    /// Color to restore when the box is not colliding.
...
fn detect_and_resolve_collisions(
    config: Res<CollisionDetectionConfig>,
...
fn bounce_walls(
    config: Res<CollisionDetectionConfig>,
    mut query: Query<(&Transform, &mut Velocity, &Box)>,
) {
...
mod tests {
    ...
    fn config_default_matches_documented_values() {
        let c = CollisionDetectionConfig::default();
        assert_eq!(c.arena_half, Vec2::new(480.0, 270.0));
    }
