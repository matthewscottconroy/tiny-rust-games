//! Dialog system — a reusable Bevy plugin.
//!
//! This crate is a *building block*: drop [`DialogSystemPlugin`] into any Bevy
//! app with `app.add_plugins(DialogSystemPlugin)` and it renders a branching
//! conversation tree with a typewriter typing effect and player choices. Tune
//! the reveal speed through the [`DialogSystemConfig`] resource without editing
...
//! - A typing timer drives `char_index` forward over time; once all
...
/// Bundles every system and resource for the dialog feature.
///
/// Add it with `app.add_plugins(DialogSystemPlugin)`. It does **not** add
/// `DefaultPlugins`, so the host app stays in control of window and rendering
/// setup.
pub struct DialogSystemPlugin;

impl Plugin for DialogSystemPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<DialogSystemConfig>()
...
/// `app.insert_resource(DialogSystemConfig { chars_per_sec: 20.0, ..default() })`.
#[derive(Resource, Clone, Copy, Debug)]
pub struct DialogSystemConfig {
...
/// Builds the demo conversation tree.
pub fn build_tree() -> Vec<DialogNode> {
    vec![
        // 0
        DialogNode {
...
pub struct DialogState {
    /// Index of the active node in the dialog tree.
    pub node_id: usize,
    /// Number of characters currently revealed.
    pub char_index: usize,
    /// Accumulated time toward the next character reveal.
    pub typing_timer: f32,
    /// Whether the current node's text is fully revealed.
    pub finished: bool,
}
...
fn tick_typing(time: Res<Time>, config: Res<DialogSystemConfig>, mut state: ResMut<DialogState>) {
    ...
    let new_chars = (state.typing_timer * config.chars_per_sec) as usize;
    state.char_index = (state.char_index + new_chars).min(total_chars);
    state.typing_timer -= new_chars as f32 / config.chars_per_sec;
}
...
mod tests {
    ...
    fn config_default_matches_documented_values() {
        let c = DialogSystemConfig::default();
        assert_eq!(c.chars_per_sec, 40.0);
    }

    #[test]
    fn setup_spawns_dialog_ui() {
        // setup does not use AssetServer, so it is safe to run headlessly.
        let mut app = App::new();
        app.add_plugins(MinimalPlugins)
            .init_resource::<DialogSystemConfig>()
            .init_resource::<DialogState>()
            .add_systems(Startup, setup);
        app.update();

        let mut q = app.world_mut().query::<&DialogUi>();
        // Speaker + Body + 3 Choices + Hint = 6 tagged UI nodes.
        assert_eq!(q.iter(app.world()).count(), 6);
    }

    #[test]
    fn plugin_composes_onto_headless_app() {
        // Demonstrates the building-block path: the plugin composes onto a
        // headless app with no extra wiring.
        let mut app = App::new();
        app.add_plugins((MinimalPlugins, DialogSystemPlugin));
        // The plugin's Update systems read input; MinimalPlugins omits it.
        app.insert_resource(ButtonInput::<KeyCode>::default());
        app.update();

        assert_eq!(app.world().resource::<DialogState>().node_id, 0);
    }
}
