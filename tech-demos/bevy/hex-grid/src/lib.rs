//! Hexagonal grid — a reusable Bevy plugin.
//!
//! This crate is a *building block*: drop [`HexGridPlugin`] into any Bevy app
//! with `app.add_plugins(HexGridPlugin)` and it spawns a hexagonal field of
//! cells; clicking a hex selects it and highlights its neighbours. Tune it
//! through the [`HexConfig`] resource without editing the plugin's internals.
//!
//! Key ideas:
...
/// Bundles every system and resource for the hex-grid feature.
///
/// Add it with `app.add_plugins(HexGridPlugin)`. It does **not** add
...
/// `app.insert_resource(HexConfig { size: 40.0, ..default() })`.
#[derive(Resource, Clone, Copy, Debug)]
pub struct HexConfig {
    /// Flat-top outer radius of a hex, in pixels.
    pub size: f32,
    /// The field extends out to this axial distance from the origin.
    pub radius: i32,
}
...
/// Axial coordinate `(q, r)` of a hex cell.
#[derive(Component, Clone, Copy, PartialEq, Eq)]
pub struct HexCoord(pub i32, pub i32);

/// Currently selected hex, if any.
#[derive(Resource)]
pub struct Selected(pub Option<(i32, i32)>);

fn setup(mut commands: Commands, config: Res<HexConfig>) {
        ...
        for r in -config.radius..=config.radius {
            ...
            commands.spawn((
                Sprite {
                    ...
                    custom_size: Some(Vec2::splat(config.size * 1.72)),
                    ..default()
                },
...
fn handle_click(
    ...
    for (coord, tf) in &hexes {
        ...
        if d < config.size * 1.1 && d < best_dist {
            best_dist = d;
...
mod tests {
    ...
    fn config_default_matches_documented_values() {
        let c = HexConfig::default();
        assert_eq!(c.size, 30.0);
