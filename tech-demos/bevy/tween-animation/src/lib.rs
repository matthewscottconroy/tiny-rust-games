//! Tween animation — a reusable Bevy plugin.
//!
//! This crate is a *building block*: drop [`TweenAnimationPlugin`] into any
//! Bevy app with `app.add_plugins(TweenAnimationPlugin)` and it demonstrates
//! three independent tween types (scale, position, alpha). Tune the tween
//! durations through the [`TweenAnimationConfig`] resource without editing the
//! plugin's internals.
//!
//! Key ideas:
...
/// Bundles every system and resource for the tween-animation demo.
///
/// Add it with `app.add_plugins(TweenAnimationPlugin)`. It does **not** add
/// `DefaultPlugins`, so the host app stays in control of window and rendering
/// setup.
pub struct TweenAnimationPlugin;

impl Plugin for TweenAnimationPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<TweenAnimationConfig>()
...
/// Tunable durations (in seconds) for the demo's tweens. Override before adding
/// the plugin, e.g.
/// `app.insert_resource(TweenAnimationConfig { scale_pop_duration: 0.5, ..default() })`.
#[derive(Resource, Clone, Copy, Debug)]
pub struct TweenAnimationConfig {
    /// Duration of the entrance slide tween.
    pub slide_duration: f32,
    /// Duration of the entrance alpha-pulse tween.
    pub entrance_alpha_duration: f32,
    /// Duration of the key-triggered scale pop.
    pub scale_pop_duration: f32,
    /// Target scale factor of the scale pop.
    pub scale_pop_to: f32,
    /// Duration of the key-triggered alpha pulse.
    pub key_alpha_duration: f32,
}

impl Default for TweenAnimationConfig {
    fn default() -> Self {
        Self {
            slide_duration: 0.7,
            entrance_alpha_duration: 1.0,
            scale_pop_duration: 0.25,
            scale_pop_to: 1.6,
            key_alpha_duration: 0.5,
        }
    }
}
...
pub struct ScaleTween {
    ...
    pub elapsed: f32,
    /// Whether the tween loops.
    pub looping: bool,
}

/// Animates an entity's X translation between `from_x` and `to_x`.
#[derive(Component)]
pub struct SlideTween {
    /// Starting X translation.
    pub from_x: f32,
    /// Ending X translation.
    pub to_x: f32,
    /// Total duration in seconds.
    pub duration: f32,
    /// Elapsed time in seconds.
...
pub struct AlphaTween {
    ...
    /// Elapsed time in seconds.
    pub elapsed: f32,
    /// Whether the tween ping-pong loops.
    pub looping: bool,
    /// The sprite's base RGB color (alpha is animated on top of it).
...
fn trigger_entrance_tweens(
    ...
    config: Res<TweenAnimationConfig>,
...
mod tests {
    ...
    fn config_default_matches_documented_values() {
        let c = TweenAnimationConfig::default();
        assert_eq!(c.slide_duration, 0.7);
        assert_eq!(c.scale_pop_to, 1.6);
    }
    ...
    fn setup_spawns_three_boxes() {
        ...
        app.add_plugins(MinimalPlugins)
            .init_resource::<TweenAnimationConfig>()
            .add_systems(Startup, (setup, trigger_entrance_tweens).chain());
        app.update();
