...
#[derive(Component)]
pub struct Player { pub apply_cd: f32 }
...
fn apply_effects(
    ...
    if dist > config.apply_range { return; }
...
fn tick_enemy(
    ...
    if enemy.wander_timer <= 0.0 {
        ...
        ).normalize_or_zero() * config.enemy_base_speed;
        enemy.wander_timer = 1.5 + (tf.translation.x.abs() % 1.0);
    }
    tf.translation += (enemy.vel * speed_mod * dt).extend(0.0);
    if tf.translation.x.abs() > config.window_w / 2.0 - 20.0 { enemy.vel.x *= -1.0; }
    if tf.translation.y.abs() > config.window_h / 2.0 - 20.0 { enemy.vel.y *= -1.0; }
    tf.translation.x = tf.translation.x.clamp(-config.window_w / 2.0 + 20.0, config.window_w / 2.0 - 20.0);
    tf.translation.y = tf.translation.y.clamp(-config.window_h / 2.0 + 20.0, config.window_h / 2.0 - 20.0);
...
fn update_hud(
    ...
    let Ok((_player, ptf)) = player_q.single() else { return };
    ...
    let dist = ptf.translation.distance(etf.translation);
    let range_note = if dist <= config.apply_range { "(in range)" } else { "(too far)" };
