...
fn setup(mut commands: Commands, seed: Res<MapSeed>) {
    commands.spawn(Camera2d);

    let origin_x = -(GRID_W as f32 * CELL_W) / 2.0 + CELL_W / 2.0;
    let origin_y =  (GRID_H as f32 * CELL_H) / 2.0 - CELL_H / 2.0;

    for row in 0..GRID_H {
        for col in 0..GRID_W {
            let v = fbm(col as f32 * SCALE, row as f32 * SCALE, seed.0, OCTAVES);
            let pos = Vec3::new(
            ...
            commands.spawn((
                Sprite {
                    color: biome_color(v),
                    custom_size: Some(Vec2::new(CELL_W - 1.0, CELL_H - 1.0)),
                    ..default()
                },
                Transform::from_translation(pos),
                MapCell(row * GRID_W + col),
            ));
        }
    }
...
fn update_colors(seed: Res<MapSeed>, mut query: Query<(&MapCell, &mut Sprite)>) {
    if !seed.is_changed() {
        return;
    }
    for (cell, mut sprite) in &mut query {
        let col = cell.0 % GRID_W;
        let row = cell.0 / GRID_W;
        let v = fbm(col as f32 * SCALE, row as f32 * SCALE, seed.0, OCTAVES);
...
mod tests {
    ...
    fn biome_color_snow_is_near_white() {
        let Color::Srgba(c) = biome_color(0.95) else { panic!("expected Srgba") };
