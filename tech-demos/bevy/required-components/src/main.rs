...
//!
//! Demonstrates Bevy 0.15+'s `#[require(...)]` attribute, which automatically
//! inserts required components with their `Default` values whenever the root
//! component is spawned. This eliminates the need for boilerplate bundle types.
//!
//! Three entity archetypes are defined:
//! - `Soldier`: requires `Health`, `Team`, and `Speed`
//! - `Mage`: requires `Health`, `ManaPool`, and `Speed`
//! - `Turret`: requires `Health` and `Team` (no `Speed` — turrets are stationary)
//!
//! Each archetype is spawned with a single component; the others arrive
//! automatically. A drain system ticks health down over time, and a HUD
//! displays live counts and average health per type.
...
/// Drains 5 HP per second from every entity that has a `Health` component.
fn drain_health(mut query: Query<&mut Health>, time: Res<Time>) {
    for mut health in &mut query {
        health.0 = (health.0 - 5.0 * time.delta_secs()).max(0.0);
    }
}
